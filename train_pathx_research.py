...
MODE = "zoh"              # Discretization: "tustin", "vanilla", or "zoh"

# Training Configuration
BATCH_SIZE = 8                # Batch size — each (B,L,D,N) tensor = 1 GB at B=8
...
# Logging
WANDB_PROJECT = "zoh-mamba-pathx"
WANDB_ENTITY = None                  # Set to your W&B username or team
USE_WANDB = True                     # Enable W&B logging

# Precision & Performance
USE_BFLOAT16 = True          # Mixed precision training
USE_GRADIENT_CHECKPOINTING = True  # Recompute activations in backward instead of
                                   # storing them — trades ~33% extra compute for
                                   # O(layers) → O(1) activation memory.  Required
                                   # on MIG instances / memory-constrained GPUs.
...
def configure_memory_management(device: torch.device):
    ...
    """
    if device.type == 'cuda':
        # Do NOT call set_per_process_memory_fraction on MIG instances.
        # The allocator enforces that limit by querying NVML before large
        # allocations; on MIG the NVML handle is for the physical GPU (not the
        # partition), so the query fails and triggers:
        #   NVML_SUCCESS == r INTERNAL ASSERT FAILED (CUDACachingAllocator.cpp)
        # Peak working set here is ~15 GB, well within the 40 GB partition —
        # no artificial cap is needed.
        #
        # Also leave PYTORCH_CUDA_ALLOC_CONF unset: expandable_segments adds
        # NVML-backed virtual memory tracking that hits the same failure path
        # during gradient-checkpointing recomputation.

        # Enable cudnn benchmarking for better performance
        torch.backends.cudnn.benchmark = True

        # TF32: H100 / A100 Tensor Cores support TF32 for float32 matmuls,
        # giving ~8× throughput vs full float32 with negligible accuracy loss.
        ...
        logging.info("  - Memory fraction: unset (MIG NVML limitation; 40 GB >> ~15 GB peak)")
        logging.info("  - CuDNN benchmark: Enabled")
        logging.info("  - TF32 (matmul + cuDNN): Enabled")
...
def collect_stability_metrics(
    ...
    - discretization_condition: ZOH exponent condition max(|Δ·A|)/min(|Δ·A|), or Tustin denominator condition for other modes
...
def collect_discretization_error(
    ...
    """
    Compute ZOH discretization error vs RK4 ground-truth.

    Analysis methodology (single-channel, first SSM layer)
    -------------------------------------------------------
    1. Run a forward pass on a short synthetic sequence to collect
       per-timestep SSM parameters (A, B_t, C_t, delta_t, x_t) from
       the first Mamba block via return_diagnostics=True.

    2. Re-run the ZOH discrete recurrence for SSM channel d=0 using the
       EXACT same parameters captured in diagnostics.  This is the
       "discrete" output.

    3. Run RK4 continuous integration for the same channel.  B_t and C_t
       are treated as piecewise-constant over [t, t+Δ_t) — the ZOH
       assumption — so the RK4 result is the exact continuous solution
       the ZOH formula approximates.

    4. Report relative L2 error and max absolute error.

    All tensors are moved to CPU and cast to float32 before the Python
    RK4 loop so the comparison is not contaminated by bfloat16 rounding.
    """
    model.eval()

    # Single-item batch; seq_len is short (default 128) for fast RK4.
    x_synth = torch.randint(0, VOCAB_SIZE, (1, seq_len), device=device)

    with torch.no_grad():
        _, diagnostics = model(x_synth, return_diagnostics=True)

        # ── Extract first-layer SSM parameters, all on CPU in float32 ──
        # diagnostics shapes:
        #   'A'      : (D_inner, N)       continuous diagonal A
        #   'A_bar'  : (1, L, D_inner, N) discrete A per step
        #   'B'      : (1, L, D_inner, N) selective B (expand view)
        #   'C'      : (1, L, D_inner, N) selective C (expand view)
        #   'delta'  : (1, L, D_inner)    step sizes
        #   'x_input': (1, L, D_inner)    SSM input sequence

        d = 0   # SSM channel index for single-channel analysis

        A_cont  = diagnostics['A'].float().cpu()             # (D_inner, N)
        A_bar_d = diagnostics['A_bar'][0, :, d, :].float().cpu()  # (L, N)
        B_seq   = diagnostics['B'][0, :, d, :].float().cpu()  # (L, N) selective
        C_seq   = diagnostics['C'][0, :, d, :].float().cpu()  # (L, N) selective
        delta_d = diagnostics['delta'][0, :, d].float().cpu() # (L,)
        x_d     = diagnostics['x_input'][0, :, d].float().cpu()  # (L,)
        A_ch    = A_cont[d]                                   # (N,)

        # ── Discrete ZOH recurrence for channel d (matches training path) ──
        # h_{t+1} = A_bar_t ⊙ h_t + B̄_t · x_t
        # B̄_t    = Δ_t · B_t · expm1(Δ_t·A) / clamp(Δ_t·A)
        h      = torch.zeros(len(A_ch), dtype=torch.float32)
        y_disc = []
        for t in range(seq_len):
            exp_arg = delta_d[t] * A_ch                       # (N,)  always ≤ 0
            B_bar_t = delta_d[t] * B_seq[t] * (
                torch.expm1(exp_arg) / exp_arg.clamp(max=-1e-10)
            )                                                 # (N,)
            h = A_bar_d[t] * h + B_bar_t * x_d[t]
            y_disc.append((C_seq[t] * h).sum().item())
        y_disc = torch.tensor(y_disc, dtype=torch.float32)   # (L,)

        # ── Continuous RK4 reference (selective B_t, C_t, ZOH assumption) ──
        error_metrics = DiscretizationErrorAnalyzer.analyze_discretization_error_selective(
            y_discrete=y_disc,
            A=A_ch,
            B_seq=B_seq,
            C_seq=C_seq,
...
def train(
    ...
    # Gradient scaler (CUDA + float16 only).
    # bfloat16 has the same 8-bit exponent as float32 and cannot underflow,
    # so GradScaler is unnecessary and adds overhead when USE_BFLOAT16=True.
    # Only enable it if someone switches to float16 in the future.
    if device.type == 'cuda' and not USE_BFLOAT16:
    ...
    logging.info(f"Gradient checkpointing: {USE_GRADIENT_CHECKPOINTING}")
        ...
        if iter_num % EVAL_INTERVAL == 0 and iter_num > 0:
            ...
            if ENABLE_STABILITY_ANALYSIS:
                try:
                    ...
                    logging.info(f"Condition number: {stability_metrics['discretization_condition']:.6f}")
            ...
            # GPU memory — report peak for THIS evaluation window only.
            # We read stats before resetting so the logged value reflects
            # the eval + stability pass, then reset for the next window.
            mem_stats = PerformanceMonitor.get_gpu_memory_usage(device)
            metric_agg.add_metrics({f'memory/{k}': v for k, v in mem_stats.items()})
            if device.type == 'cuda':
                logging.info(f"VRAM peak (eval window): {mem_stats['vram_peak_mb']:.1f} MB")
            ...
            # Return all evaluation memory to the CUDA driver and reset the
            # peak counter so the next training window's VRAM stat is clean.
...
def main():
    ...
    # torch.compile is skipped when gradient checkpointing is active.
    #
    # Why: torch.compile wraps the model in AOT autograd, which manages tensor
    # saves through its own internal mechanism.  This overrides
    # saved_tensors_hooks — the exact hook that gradient checkpointing
    # (use_reentrant=False) relies on to intercept and release each block's
    # scan tensors (h_states, A_bar) between blocks.  Without the hook,
    # those 2 GB tensors accumulate for all 6 blocks during the forward pass
    # (~24 GB), and backward OOMs trying to allocate the recomputation tensors
    # on top of them.
    #
    # In pure eager mode the hook works correctly: each block's scan tensors
    # are released as soon as the block's checkpoint completes, keeping peak
    # memory at ~8 GB (one block at a time) regardless of depth.
    if USE_GRADIENT_CHECKPOINTING:
        logging.info("Gradient checkpointing enabled — skipping torch.compile "
                     "(AOT autograd overrides saved_tensors_hooks; "
                     "eager mode required for correct memory release)")
    else:
        try:
